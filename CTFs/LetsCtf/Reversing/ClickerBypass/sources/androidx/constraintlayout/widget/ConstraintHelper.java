...
public abstract class ConstraintHelper extends View {
    ...
    private void addID(String str) {
        if (str != null && str.length() != 0 && this.myContext != null) {
            ...
            Log.w("ConstraintHelper", new StringBuilder(String.valueOf(trim).length() + 23).append("Could not find id of \"").append(trim).append("\"").toString());
        }
    }
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if ((layoutParams instanceof ConstraintLayout.LayoutParams) && trim.equals(((ConstraintLayout.LayoutParams) layoutParams).constraintTag)) {
                    if (childAt.getId() == -1) {
                        ...
                        Log.w("ConstraintHelper", new StringBuilder(String.valueOf(simpleName).length() + 42).append("to use ConstraintTag view ").append(simpleName).append(" must have an ID").toString());
                    } else {
