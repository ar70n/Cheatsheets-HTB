...
public class MotionLabel extends View implements FloatLayout {
    ...
    public void buildShape(float f) {
        if (this.mUseOutline || f != 1.0f) {
            ...
            if (f != 1.0f) {
                ...
                Log.v(str2, new StringBuilder(String.valueOf(loc).length() + 22).append(loc).append(" scale ").append(f).toString());
    ...
    public void setTextSize(float f) {
        ...
        String str = TAG;
        String loc = Debug.getLoc();
        Log.v(str, new StringBuilder(String.valueOf(loc).length() + 35).append(loc).append("  ").append(f).append(" / ").append(this.mBaseTextSize).toString());
        TextPaint textPaint = this.mPaint;
